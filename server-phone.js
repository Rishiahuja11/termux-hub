...
async function searchAPKPure(q) {
  const strategies = [
    ...
    async () => {
      const r = await runCmd(`curl -sL "https://search.brave.com/search?q=apkpure.com+${q.replace(/'/g, '')}" -H "User-Agent: Mozilla/5.0" 2>&1`, 12);
      const pkgs = new Set();
      for (const m of (r.stdout || '').matchAll(/apkpure\.com\/[a-z0-9-]+\/([a-z][a-z0-9_.]+)/gi)) {
        if (m[1].includes('.') && /^[a-z]/.test(m[1]) && m[1].length < 80) pkgs.add(m[1]);
      }
  ...
  for (const pkg of pkgs.slice(0, 15)) {
    try {
      ...
      results.push({ id: pkg, name: detail.name || pkg, summary: detail.description ? detail.description.substring(0, 120) : '', icon: detail.icon || '', source: 'apkpure', version: detail.version || '', size: detail.size ? String(detail.size) : '', apkUrl: detail.apkUrl || '' });
    } catch (_) {
      results.push({ id: pkg, name: pkg, summary: '', icon: '', source: 'apkpure', version: '', size: '', apkUrl: '' });
    }
  }
  return results;
}
...
async function route(req, res) {
  ...
  if (pathname === '/api/auth/create' && method === 'POST') {
    ...
    // Allow setting password for placeholder admin account (initial setup)
    if (users[username] && users[username].hash === 'placeholder') {
      users[username] = { ...hashPassword(password), created: Date.now() };
      saveUsers(users);
      const token = createSession(username);
      log(`account-setup user=${username} ip=${ip}`);
      return sendJson(res, 200, { ok: true, token, username });
    }
    if (users[username]) return sendJson(res, 409, { ok: false, error: 'username already exists' });
    users[username] = { ...hashPassword(password), created: Date.now() };
    saveUsers(users);
    const token = createSession(username);
  ...
  // MJPEG live stream: adb screenrecord H264 -> ffmpeg -> MJPEG multipart
  if (pathname === '/api/android/stream' && method === 'GET') {
    if (!adbConnected()) return sendJson(res, 503, { ok: false, error: 'ADB not connected' });
    const FFMPEG = '/data/data/com.termux/files/usr/bin/ffmpeg';
    const cfg = loadConfig();
    const boundary = 'frame';
    const [sw, sh] = (cfg.streamResolution || '720x1280').split('x').map(Number);

    // Test if screenrecord stdout works (some devices don't support it)
    const testRec = await runCmd(`timeout 2 "${ADB_BIN}" shell "screenrecord --output-format=h264 --time-limit 1 --size 360x640 -" 2>/dev/null | wc -c`, 8);
    const screenrecordWorks = parseInt((testRec.stdout || '0').trim()) > 5000;

    if (screenrecordWorks) {
      // Use screenrecord -> ffmpeg pipeline
      res.writeHead(200, {
      ...
      const screenrecord = spawn(ADB_BIN, ['shell', 'screenrecord', '--output-format=h264', '--bit-rate=' + (cfg.streamBitrate || 2000000), '--size=' + (sw || 720) + 'x' + (sh || 1280), '--time-limit=180', '-'], { env: process.env, stdio: ['ignore', 'pipe', 'pipe'] });
      const ffmpeg = spawn(FFMPEG, ['-f', 'h264', '-i', 'pipe:0', '-f', 'mjpeg', '-q:v', String(cfg.streamQuality || 8), '-r', String(cfg.streamFps || 15), '-an', 'pipe:1'], { env: process.env, stdio: ['pipe', 'pipe', 'pipe'] });
      screenrecord.stdout.pipe(ffmpeg.stdin);
      let alive = true;
      const cleanup = () => { alive = false; try { screenrecord.kill('SIGTERM'); } catch (_) {} try { ffmpeg.kill('SIGTERM'); } catch (_) {} };
      screenrecord.on('error', cleanup);
      ffmpeg.on('error', cleanup);
      screenrecord.on('close', () => { try { ffmpeg.kill('SIGTERM'); } catch (_) {} });
      ffmpeg.on('close', () => { if (alive) res.end(); });
      ...
      screenrecord.stderr.on('data', () => {});
      log(`android-stream start method=screenrecord+ffmpeg ip=${ip}`);
    } else {
      // Fallback: fast screenshot polling via screencap + ffmpeg
      res.writeHead(200, {
        'Content-Type': 'multipart/x-mixed-replace; boundary=' + boundary,
        'Cache-Control': 'no-cache, no-store, must-revalidate',
        'Connection': 'keep-alive',
      ...
      req.on('close', cleanup);
      const fps = cfg.streamFps || 10;
      const delay = Math.round(1000 / fps);
      const poll = async () => {
        if (!alive || !res.writable) return;
        try {
          const buf = await new Promise((resolve, reject) => {
            execFile(ADB_BIN, ['exec-out', 'screencap', '-p'], { encoding: 'buffer', timeout: 10000, maxBuffer: 16 * 1024 * 1024, env: process.env },
              (err, stdout) => err ? reject(err) : resolve(stdout));
          });
          if (buf && buf.length > 100 && alive && res.writable) {
            res.write('--' + boundary + '\r\nContent-Type: image/png\r\nContent-Length: ' + buf.length + '\r\n\r\n');
            res.write(buf);
            res.write('\r\n');
          }
        } catch (_) {}
        if (alive && res.writable) setTimeout(poll, delay);
      };
      log(`android-stream start method=screencap-poll ip=${ip}`);
      poll();
    }
    return;
  }
  ...
  if (pathname === '/api/android/input' && method === 'POST') {
    ...
    else if (type === 'key') cmd = `adb shell input keyevent ${body.keycode || body.key}`;
    else if (type === 'text') cmd = `adb shell input text "${String(body.text).replace(/"/g, '\\"')}"`;
  ...
  if (pathname === '/api/android/download-apk' && method === 'POST') {
    ...
    const r = await runCmd(`curl -L -o "${dest}" "${urlStr}" 2>&1`, 120);
  ...
  if (pathname === '/api/android/install-apk-url' && method === 'POST') {
    ...
    const dest = path.join(apkDir, filename);
    const dl = await runCmd(`curl -L -s -o "${dest}" "${urlStr}" 2>&1`, 180);
    const size = fs.existsSync(dest) ? fs.statSync(dest).size : 0;
  ...
  if (pathname === '/api/config' && method === 'POST') {
    ...
    const allowed = ['adbTarget', 'streamBitrate', 'streamResolution', 'streamFps', 'streamQuality', 'hostname', 'port', 'adminNote'];
  ...
  if (pathname === '/api/config/restart' && method === 'POST') {
    ...
    return sendJson(res, 200, { ok: true, message: 'restart scheduled' });
  }
