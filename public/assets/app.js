...
async function renderSettings(el) {
  ...
  el.innerHTML = `<div style="max-width:520px">
    ...
    <div class="ctrl-section"><h3>SSH Access</h3><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div><label style="font-size:12px;color:var(--fg2)">SSH IP</label><input id="cfg-ssh-ip" type="text" value="${escH(cfg.sshIp || '')}" placeholder="e.g. 192.168.1.25" style="margin-top:4px"></div><div><label style="font-size:12px;color:var(--fg2)">SSH Port</label><input id="cfg-ssh-port" type="number" value="${cfg.sshPort || 8022}" min="1" max="65535" style="margin-top:4px"></div></div><div style="margin-top:10px"><label style="font-size:12px;color:var(--fg2)">SSH Password</label><input id="cfg-ssh-pw" type="password" value="${escH(cfg.sshPassword || '')}" placeholder="SSH password" style="margin-top:4px"></div></div>
...
async function saveConfig() {
  const cfg = {
    sshIp: $('#cfg-ssh-ip')?.value || '',
    sshPort: parseInt($('#cfg-ssh-port')?.value || '8022'),
