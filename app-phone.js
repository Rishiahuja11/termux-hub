...
// ── Auth ──
function showGate() {
  const users = {}; // will be determined by server
  const gate = $('#gate');
...
function appTile(a) {
  ...
  const iconContent = a.icon_url ? `<img src="${escH(a.icon_url)}" onerror="this.parentElement.innerHTML='${a.icon || '&#9881;'}'">` : (a.icon || '&#9881;');
  ...
  if (!st.adb) {
    ctrl.innerHTML = `
      <div class="ctrl-section" style="border-color:var(--warn)">
        ...
        <p style="font-size:13px;color:var(--fg2);margin-bottom:12px">Live screen and remote input need wireless debugging. Enter your device's wireless-debugging address below.</p>
        <div style="display:flex;gap:8px">
          <input class="inp" id="adb-target" placeholder="192.168.1.5:43355">
...
async function androidTap() { const x = parseInt($('#tap-x')?.value); const y = parseInt($('#tap-y')?.value); if (isNaN(x) || isNaN(y)) return toast('Invalid coordinates', 'error'); try { await api('/api/android/input', { method: 'POST', body: JSON.stringify({ type: 'tap', x, y }) }); } catch (_) {} }
        ...
        <div style="display:flex;gap:10px">
          ...
          <div style="flex:1">
            ...
            <select class="inp" id="cfg-fps" style="width:100%;margin-top:4px">
              <option value="10" ${cfg.streamFps==10?'selected':''}>10 fps (Low latency)</option>
